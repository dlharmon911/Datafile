#ifndef _HEADER_GUARD_ALLEGRO_DATAFILE_ADDON_H_
#define _HEADER_GUARD_ALLEGRO_DATAFILE_ADDON_H_
...
/**
 * @brief Function pointer type for loading a datafile object from a file.
 * @param f File handle to read from.
 * @return Pointer to the loaded object, or NULL on failure.
 */
typedef void* (*ALLEGRO_DATAFILE_TYPE_LOADER_FUNC)(ALLEGRO_FILE* f);

/**
 * @brief Function pointer type for saving a datafile object to a file.
 * @param f File handle to write to.
 * @param data Pointer to the object to save.
 * @return 0 on success, -1 on failure.
 */
typedef int32_t(*ALLEGRO_DATAFILE_TYPE_SAVER_FUNC)(ALLEGRO_FILE* f, void* data);

/**
 * @brief Function pointer type for destroying/freeing a datafile object.
...
/**
 * @brief Function pointer type for rendering a datafile object.
 * @param data Pointer to the object to render.
 * @return 0 on success, -1 on failure.
 */
typedef int32_t(*ALLEGRO_DATAFILE_TYPE_RENDERER_FUNC)(void* data);

/**
 * @brief Function pointer type for loading an object from a file without additional arguments.
...
/**
 * @brief Function pointer type for loading an object from a file with additional arguments.
 * @param file File handle to read from.
 * @param data Pointer to additional arguments/configuration data.
 * @return Pointer to the loaded object, or NULL on failure.
 */
...
/**
 * @defgroup DatafileStructs Datafile Structures
 * @{
 */

/**
 * @brief Virtual function table for datafile object type handlers.
 * 
 * This structure defines the operations that can be performed on a specific
 * datafile object type.
 */
typedef struct ALLEGRO_DATAFILE_TYPE_VTABLE
{
	...
	ALLEGRO_DATAFILE_TYPE_RENDERER_FUNC render;   /**< Function to render the object */
} ALLEGRO_DATAFILE_TYPE_VTABLE;

/**
 * @brief Container for raw binary data.
 * 
 * This structure stores arbitrary binary data with a size field followed by
 * the actual data bytes in a flexible array member.
 */
typedef struct ALLEGRO_DATA
{
	size_t size;      /**< Size of the data in bytes */
	uint8_t data[];   /**< Flexible array member containing the actual data */
} ALLEGRO_DATA;

/**
 * @brief Container for an array of bitmap sub-regions.
 * 
 * This structure represents a bitmap that has been subdivided into a grid
 * of smaller sub-bitmaps, useful for sprite sheets or tile maps.
 */
typedef struct ALLEGRO_BITMAP_ARRAY
{
	...
	int32_t height;                /**< Height of each sub-bitmap in pixels */
} ALLEGRO_BITMAP_ARRAY;

/**
 * @brief Container for a single datafile object entry.
 * 
 * This structure represents one object stored in a datafile, containing
 * a pointer to the data and its type identifier.
 */
typedef struct ALLEGRO_DATAFILE
{
	void* data;      /**< Pointer to the actual object data */
	uint32_t type;   /**< Type identifier (e.g., ALLEGRO_DATAFILE_TYPE_BITMAP) */
} ALLEGRO_DATAFILE;

/** @} */

/**
 * @defgroup DatafileCore Core Datafile Functions
 * @{
 */

/**
 * @brief Gets the version number of the datafile addon.
 * @return Version number as a 32-bit unsigned integer.
 */
uint32_t al_get_datafile_addon_version(void);

/**
 * @brief Checks if the datafile addon has been initialized.
 * @return true if initialized, false otherwise.
 */
bool al_is_datafile_addon_initialized(void);

/**
 * @brief Initializes the datafile addon.
 * 
 * This function must be called before using any other datafile functions.
 * It initializes all required Allegro subsystems and registers default object types.
 * 
...
/**
 * @brief Shuts down the datafile addon and frees all resources.
 * 
 * After calling this function, the addon must be reinitialized before use.
 */
void al_shutdown_datafile_addon(void);

/**
 * @brief Registers a custom datafile object type handler.
 * 
 * This allows you to add support for custom object types in datafiles.
 * 
 * @param type The type identifier for this object type (use AL_ID macro).
 * @param vtable Pointer to the virtual function table defining the type's operations.
 * @return true on success, false on failure.
 */
bool al_register_datafile_object_type(uint32_t type, const ALLEGRO_DATAFILE_TYPE_VTABLE* vtable);

/**
 * @brief Creates a new empty datafile.
 * @return Pointer to the new datafile, or NULL on failure.
 */
ALLEGRO_DATAFILE* al_create_datafile(void);

/**
 * @brief Loads a datafile from disk.
 * 
 * The file can be compressed or uncompressed. This function automatically
 * handles decompression if needed.
 * 
 * @param filename Path to the datafile to load.
 * @return Pointer to the loaded datafile, or NULL on failure.
 */
 ...
 * 
 * @param datafile Pointer to the datafile to destroy.
 */
void al_destroy_datafile(ALLEGRO_DATAFILE* datafile);

/**
 * @brief Gets the number of objects stored in a datafile.
 * @param datafile Pointer to the datafile.
 * @return Number of objects in the datafile, or 0 if datafile is NULL.
 */
size_t al_get_datafile_object_count(const ALLEGRO_DATAFILE* datafile);

/**
 * @brief Gets the name of an object in the datafile by index.
 * @param datafile Pointer to the datafile.
 ...
 * 
 * Note: This function may reallocate the datafile structure, so always use
 * the pointer passed by reference.
 * 
 * @param datafile Pointer to pointer to the datafile (may be modified).
 * @param type Type identifier for the object.
...
/**
 * @brief Loads an object from a file and adds it to the datafile.
 * 
 * @param datafile Pointer to pointer to the datafile (may be modified).
 * @param type Type identifier for the object.
 * @param name Name/identifier for the object.
 * @param filename Path to the file containing the object.
 * @param loader Function to use for loading the object.
 * @return 0 on success, -1 on failure.
 */
...
/**
 * @brief Loads an object from a file with additional arguments and adds it to the datafile.
 * 
 * @param datafile Pointer to pointer to the datafile (may be modified).
 * @param type Type identifier for the object.
 * @param name Name/identifier for the object.
 * @param filename Path to the file containing the object.
 * @param loader Function to use for loading the object.
 * @param args Pointer to additional arguments for the loader function.
 * @return 0 on success, -1 on failure.
 */
...
/**
 * @defgroup DatafileHelperStructs Helper Structures for Loading
 * @{
 */

/**
 * @brief Configuration data for loading bitmap arrays.
 * 
 * This structure specifies how to divide a source bitmap into a grid
 * of sub-bitmaps.
 */
...
/**
 * @brief Configuration data for loading bitmap fonts.
 * 
 * This structure specifies the character ranges to extract from a
 * bitmap font image.
 */
...
/**
 * @brief Configuration data for loading TrueType fonts.
 * 
 * This structure specifies the parameters for loading a TTF font file.
 */
typedef struct ALLEGRO_DATAFILE_TTF_FONT_DATA
{
	const char* filename;  /**< Path to the TTF font file */
	int size;              /**< Font size in pixels */
	int flags;             /**< Font loading flags (see Allegro font addon) */
} ALLEGRO_DATAFILE_TTF_FONT_DATA;

/** @} */

/**
 * @defgroup DatafileLoaders Object Loader Functions
 * @{
 */
...
/**
 * @brief Loads a bitmap array from an open file handle.
 * 
 * This function loads a bitmap and divides it into a grid of sub-bitmaps
 * according to the parameters in the args structure.
 * 
 * @param file Open file handle positioned at the bitmap data.
 * @param args Pointer to configuration data specifying grid dimensions.
 * @return Pointer to the loaded bitmap array, or NULL on failure.
 */
...
/**
 * @brief Loads a TrueType font from an open file handle.
 * 
 * @param file Open file handle positioned at the TTF font data.
 * @param args Pointer to configuration data specifying font size and flags.
...
/**
 * @brief Loads a bitmap font from an open file handle.
 * 
 * This function loads a bitmap containing font glyphs and extracts the
 * specified character ranges.
 * 
 * @param file Open file handle positioned at the bitmap font data.
 * @param args Pointer to configuration data specifying character ranges.
 * @return Pointer to the loaded font, or NULL on failure.
 */
ALLEGRO_FONT* al_load_datafile_bitmap_font_f(ALLEGRO_FILE* file, const ALLEGRO_DATAFILE_BITMAP_FONT_DATA* args);

/**
 * @brief Loads a text string from an open file handle.
 * 
 * The text is loaded as a UTF-8 encoded string.
 * 
 * @param file Open file handle positioned at the text data.
 * @return Pointer to the loaded string (ALLEGRO_USTR), or NULL on failure.
 */
ALLEGRO_USTR* al_load_datafile_text_f(ALLEGRO_FILE* file);

/**
 * @brief Loads raw binary data from an open file handle.
 * 
 * @param file Open file handle positioned at the data.
 * @return Pointer to the loaded data structure, or NULL on failure.
 */
...
#ifdef ALLEGRO_DATAFILE_IMPLENTATION
...
#endif // !ALLEGRO_DATAFILE_IMPLENTATION


#endif // !_HEADER_GUARD_ALLEGRO_DATAFILE_ADDON_H_